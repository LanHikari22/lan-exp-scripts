...
Proposition example_z_trichotomy_3_again_with_nat_ord : forall a_ _a : nat, 
  ({| pos := a_; neg := _a |} ~= (NZ#0)) \/ 
...
Proposition z_non_neg_add_preserves_ne : forall a b : nat,
  (((NZ#a) ~= (NZ#0))%Z -> False) -> (((NZ#b) ~= (NZ#0))%Z -> False) ->
  (((NZ#a+b) ~= (NZ#0))%Z -> False).
Proof.
  intros a.
  ...
  }
  {
    ...
    destruct b as [| b'].
    {
      rewrite -> add_0_r in Hnz_ab.
      rewrite -> z_nz_inject in Hnz_ab.
      discriminate.
    }
    {
      rewrite -> z_nz_inject in Hnz_ab.
      rewrite -> add_Sn_m in Hnz_ab.
      discriminate.
    }
  }
Qed.
...
Proposition z_nz_ne_inject : forall (a b : nat), 
  (((NZ#a) ~= (NZ#b))%Z -> False) <-> ((a=b) -> False).
Proof.
  intros a b.
  split.
  {
    intros Hnzab1 Hnzab2.
    apply <- z_nz_inject in Hnzab2.
    specialize (Hnzab1 Hnzab2).
    contradiction.
  }
  {
    intros Hnzab1 Hnzab2.
